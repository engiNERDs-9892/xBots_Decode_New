...
public class Arm {
  ...
  private final int limitSlide = 4200, limitPivot = 75;
  private final double countsPerDegree = 41.855, countsPerInch = 120.88;
  ...
  private int pivotTarget = 0, slideTarget = 0;
  private double pivotPower = 0, slidePower = 0;
  ...
  public void setPivot(int x) {
    ...
    //    maxLength = limitSlide * Math.cos(Math.toRadians(pivot.getCurrentPosition() /
    // countsPerDegree)) * 1;
    //    if(maxLength < 2500)
    //      maxLength = 2500;
  ...
  public void setPivot(double power) {
    ...
    maxLength =
        limitSlide * Math.cos(Math.toRadians(pivot.getCurrentPosition() / countsPerDegree)) * 1;
  ...
  public void setSlide(int x) {
    ...
    maxLength =
        limitSlide * Math.cos(Math.toRadians(pivot.getCurrentPosition() / countsPerDegree)) * 1.4;
    if (maxLength < 2625) maxLength = 2625;

    if (maxLength > limitSlide) maxLength = limitSlide;

    // Clamps the input to maxLength and 0
    if (x > maxLength) x = (int) maxLength;
    if (x < 0) x = 0;
  ...
  public void setSlide(double power) {
    ...
    // Calculates the max the arm can go without going over the 40IN limit
    double maxLength =
        limitSlide * Math.cos(Math.toRadians(pivot.getCurrentPosition() / countsPerDegree)) * 1.2;
    if (maxLength < 2625) maxLength = 2625;
    ...
    if (slide.getCurrentPosition() <= 0 && power <= 0) power = -0.25;
